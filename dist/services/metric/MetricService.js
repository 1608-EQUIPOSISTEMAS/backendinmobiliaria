...
class MetricService extends BaseRepository_1.BaseRepository {
    constructor() {
        super();
    }
    /**
    ...
    async getDashboard() {
        try {
            ...
            // 1. Resumen general
            const resumen = await this.queryOne(`
        SELECT 
      ...
      `, [hoy]);
            ...
            const [porEstado] = await this.query(`
        SELECT 
          et.nombre as estado,
          et.codigo,
          et.color,
      ...
      `, [hoy]);
            // 3. SLA en riesgo
            const [slaEnRiesgo] = await this.query(`
        SELECT 
          t.id,
          t.codigo,
          t.titulo,
      ...
      `);
            ...
            const [ultimosTickets] = await this.query(`
        SELECT 
          t.id,
          t.codigo,
          t.titulo,
      ...
      `);
            ...
            logger_util_1.logger.error('❌ Error al obtener dashboard:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getTechnicianMetrics(tecnicoId, fechaInicio, fechaFin) {
    ...
    `, [tecnicoId, inicio, fin]);
        ...
        const [porCategoria] = await this.query(`
      SELECT 
    ...
    `, [tecnicoId, inicio, fin]);
        ...
        const [porPrioridad] = await this.query(`
      SELECT 
        p.nombre as prioridad,
        p.nivel,
        COUNT(*) as total
    ...
    `, [areaId, inicio, fin]);
        // Tickets por tipo
        const [porTipo] = await this.query(`
      SELECT 
        tt.nombre as tipo,
        COUNT(*) as total
      FROM tickets t
    ...
    `, [areaId, inicio, fin]);
        ...
        const [topProblemas] = await this.query(`
      SELECT 
        ct.nombre as categoria,
        COUNT(*) as frecuencia,
     ...
     */
    async getCategoryStats(fechaInicio, fechaFin) {
        const inicio = fechaInicio || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        const fin = fechaFin || new Date().toISOString().split('T')[0];
     ...
     */
    async getSlaStats(fechaInicio, fechaFin) {
        const inicio = fechaInicio || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        const fin = fechaFin || new Date().toISOString().split('T')[0];
        const general = await this.queryOne(`
      SELECT 
        COUNT(*) as total_tickets,
     ...
     */
    async getSatisfactionStats(fechaInicio, fechaFin) {
        const inicio = fechaInicio || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
        const fin = fechaFin || new Date().toISOString().split('T')[0];
        const general = await this.queryOne(`
      SELECT 
    ...
    `, [inicio, fin]);
        ...
        const [porTecnico] = await this.query(`
      SELECT 
        u.id,
        CONCAT(u.nombre, ' ', u.apellido) as tecnico,
        COUNT(*) as total_encuestas,
     ...
     */
    async comparePeriodsMetrics(periodo1Inicio, periodo1Fin, periodo2Inicio, periodo2Fin) {
        const periodo1 = await this.queryOne(`
      SELECT 
        COUNT(*) as total_tickets,
        COUNT(CASE WHEN et.codigo IN ('RESUELTA', 'CERRADA') THEN 1 END) as resueltos,
        ROUND(AVG(TIMESTAMPDIFF(MINUTE, t.created_at, t.fecha_resolucion)), 0) as tiempo_promedio,
    ...
    `, [periodo1Inicio, periodo1Fin]);
        const periodo2 = await this.queryOne(`
      SELECT 
        COUNT(*) as total_tickets,
        COUNT(CASE WHEN et.codigo IN ('RESUELTA', 'CERRADA') THEN 1 END) as resueltos,
        ROUND(AVG(TIMESTAMPDIFF(MINUTE, t.created_at, t.fecha_resolucion)), 0) as tiempo_promedio,
