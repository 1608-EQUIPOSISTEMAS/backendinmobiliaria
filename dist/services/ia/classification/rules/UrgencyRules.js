...
class UrgencyRules {
    ...
    detectUrgency(text) {
        ...
        const criticaMatches = this.keywordExtractor.findMatchingKeywords(text, keywords_constant_1.URGENCY_KEYWORDS.CRITICA);
        scores.critica = criticaMatches.length * 4;
        // Buscar keywords de urgencia alta
        const altaMatches = this.keywordExtractor.findMatchingKeywords(text, keywords_constant_1.URGENCY_KEYWORDS.ALTA);
        scores.alta = altaMatches.length * 3;
        // Buscar keywords de urgencia media
        const mediaMatches = this.keywordExtractor.findMatchingKeywords(text, keywords_constant_1.URGENCY_KEYWORDS.MEDIA);
        scores.media = mediaMatches.length * 2;
        // Buscar keywords de urgencia baja
        const bajaMatches = this.keywordExtractor.findMatchingKeywords(text, keywords_constant_1.URGENCY_KEYWORDS.BAJA);
        scores.baja = bajaMatches.length * 1;
        // Determinar urgencia con mayor score
        const maxScore = Math.max(scores.critica, scores.alta, scores.media, scores.baja);
        let urgencyLevel;
        let matchedKeywords;
        if (maxScore === 0) {
            // Sin keywords específicos, asumir urgencia media
            urgencyLevel = priorities_constant_1.URGENCY_LEVELS.MEDIA;
            matchedKeywords = [];
        }
        else if (scores.critica === maxScore) {
            urgencyLevel = priorities_constant_1.URGENCY_LEVELS.CRITICA;
            matchedKeywords = criticaMatches;
        }
        else if (scores.alta === maxScore) {
            urgencyLevel = priorities_constant_1.URGENCY_LEVELS.ALTA;
            ...
            urgencyLevel = priorities_constant_1.URGENCY_LEVELS.BAJA;
...
exports.UrgencyRules = UrgencyRules;
//# sourceMappingURL=UrgencyRules.js.map