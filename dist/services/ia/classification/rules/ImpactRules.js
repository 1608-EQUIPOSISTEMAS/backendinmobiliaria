...
class ImpactRules {
    ...
    detectImpact(text) {
        ...
        const criticoMatches = this.keywordExtractor.findMatchingKeywords(text, keywords_constant_1.IMPACT_KEYWORDS.CRITICO);
        scores.critico = criticoMatches.length * 4;
        // Buscar keywords de impacto alto
        const altoMatches = this.keywordExtractor.findMatchingKeywords(text, keywords_constant_1.IMPACT_KEYWORDS.ALTO);
        scores.alto = altoMatches.length * 3;
        // Buscar keywords de impacto medio
        const medioMatches = this.keywordExtractor.findMatchingKeywords(text, keywords_constant_1.IMPACT_KEYWORDS.MEDIO);
        scores.medio = medioMatches.length * 2;
        // Buscar keywords de impacto bajo
        const bajoMatches = this.keywordExtractor.findMatchingKeywords(text, keywords_constant_1.IMPACT_KEYWORDS.BAJO);
        scores.bajo = bajoMatches.length * 1;
        // Determinar impacto con mayor score
        const maxScore = Math.max(scores.critico, scores.alto, scores.medio, scores.bajo);
        let impactLevel;
        let matchedKeywords;
        if (maxScore === 0) {
            // Sin keywords específicos, asumir impacto medio
            impactLevel = priorities_constant_1.IMPACT_LEVELS.MEDIO;
            matchedKeywords = [];
        }
        else if (scores.critico === maxScore) {
            impactLevel = priorities_constant_1.IMPACT_LEVELS.CRITICO;
            matchedKeywords = criticoMatches;
        }
        else if (scores.alto === maxScore) {
            impactLevel = priorities_constant_1.IMPACT_LEVELS.ALTO;
            ...
            impactLevel = priorities_constant_1.IMPACT_LEVELS.BAJO;
...
exports.ImpactRules = ImpactRules;
//# sourceMappingURL=ImpactRules.js.map