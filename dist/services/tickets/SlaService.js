...
class SlaService extends BaseRepository_1.BaseRepository {
    constructor() {
        super();
    }
    /**
     ...
     */
    async calculateAndSaveSla(ticketId, urgenciaId, impactoId) {
        try {
            // Obtener configuración de SLA
            const slaConfig = await this.queryOne(`
        SELECT 
          id,
      ...
      `, [urgenciaId, impactoId]);
            if (!slaConfig) {
                logger_util_1.logger.warn(`⚠️ No se encontró configuración SLA para urgencia ${urgenciaId} e impacto ${impactoId}`);
                return;
            }
            const now = new Date();
            // Calcular fechas límite
            const fechaLimiteRespuesta = (0, date_util_1.addMinutes)(now, slaConfig.tiempo_respuesta_minutos);
            const fechaLimiteResolucion = (0, date_util_1.addMinutes)(now, slaConfig.tiempo_resolucion_minutos);
            const fechaAlertaRespuesta = (0, date_util_1.addMinutes)(now, slaConfig.tiempo_respuesta_minutos - slaConfig.tiempo_alerta_respuesta);
            const fechaAlertaResolucion = (0, date_util_1.addMinutes)(now, slaConfig.tiempo_resolucion_minutos - slaConfig.tiempo_alerta_resolucion);
      ...
      `, [
            ...
            logger_util_1.logger.info(`✅ SLA calculado para ticket ${ticketId}`, {
                respuesta: `${slaConfig.tiempo_respuesta_minutos} minutos`,
                resolucion: `${slaConfig.tiempo_resolucion_minutos} minutos`,
            });
        }
        catch (error) {
            logger_util_1.logger.error('❌ Error al calcular SLA:', error);
            throw error;
        }
    }
    /**
    ...
    async checkSlaAlerts() {
        ...
        const alertasRespuesta = await this.query(`
      SELECT 
        s.ticket_id,
        t.codigo,
        t.titulo,
    ...
    `);
        ...
        const alertasResolucion = await this.query(`
      SELECT 
        s.ticket_id,
        t.codigo,
        t.titulo,
    ...
    `);
        ...
        const alertasVencidas = await this.query(`
      SELECT 
        s.ticket_id,
        t.codigo,
        t.titulo,
     ...
     */
    async registerSlaAlert(ticketId, tipoAlerta, destinatarios) {
        await this.query(`
     ...
     */
    async updateFirstResponse(ticketId) {
        await this.query(`
     ...
     */
    async updateResolution(ticketId) {
        await this.query(`
      UPDATE sla_seguimiento
      SET 
