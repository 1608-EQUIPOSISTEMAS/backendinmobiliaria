...
export class MetricRepository extends BaseRepository<any> {
  /**
   * Obtener métricas generales del sistema
   */
  async getGeneralMetrics(fechaInicio?: string, fechaFin?: string): Promise<any> {
    let dateFilter = '';
    const params: any[] = [];
   ...
   * Métricas por área
   */
  async getMetricsByArea(fechaInicio?: string, fechaFin?: string): Promise<any[]> {
    let dateFilter = '';
    const params: any[] = [];
   ...
   */
  async getMetricsByTechnician(fechaInicio?: string, fechaFin?: string): Promise<any[]> {
    let dateFilter = '';
    const params: any[] = [];
   ...
   * Métricas de SLA
   */
  async getSLAMetrics(fechaInicio?: string, fechaFin?: string): Promise<any> {
    let dateFilter = '';
    const params: any[] = [];
   ...
   */
  async getTicketsByCategory(fechaInicio?: string, fechaFin?: string): Promise<any[]> {
    let dateFilter = '';
    const params: any[] = [];
   ...
   */
  async getTicketsByPriority(fechaInicio?: string, fechaFin?: string): Promise<any[]> {
    let dateFilter = '';
    const params: any[] = [];
   ...
   */
  async getTicketsTrend(days = 30): Promise<any[]> {
    const [trend] = await this.query<any[]>(`
      SELECT 
   ...
   */
  async getTopTechnicians(limit = 10): Promise<any[]> {
    const [technicians] = await this.query<any[]>(`
      SELECT 
        u.id,
        CONCAT(u.nombre, ' ', u.apellido) as tecnico,
   ...
   */
  async getTechnicianMetricsByPeriod(
    tecnicoId: number,
    fechaInicio: string,
    fechaFin: string
  ): Promise<any> {
  ...
  async getPerformanceAlerts(): Promise<any[]> {
    const [alerts] = await this.query<any[]>(`
      SELECT 
   ...
   */
  private calculateVariation(valorAnterior: number, valorActual: number): any {
