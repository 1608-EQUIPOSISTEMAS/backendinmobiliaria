...
export class ReportService extends BaseRepository<any> {
  constructor() {
    super();
  }
  ...
  async generateManagerialReport(
    fechaInicio: string,
    fechaFin: string,
    tipoReporte: 'diario' | 'semanal' | 'mensual' | 'trimestral' | 'anual' | 'custom' = 'mensual'
  ): Promise<any> {
    try {
      logger.info(`📊 Generando reporte gerencial ${tipoReporte}`, {
      ...
      logger.info('✅ Reporte gerencial generado exitosamente');

      return report;
    } catch (error: any) {
      logger.error('❌ Error al generar reporte gerencial:', error);
      throw error;
    }
  }
   ...
   */
  private async getGeneralMetrics(fechaInicio: string, fechaFin: string): Promise<any> {
   ...
   */
  private async getSlaCompliance(fechaInicio: string, fechaFin: string): Promise<any> {
    const result = await this.queryOne<any>(`
      SELECT 
        COUNT(*) as total_tickets,
   ...
   */
  private async getTicketsByState(fechaInicio: string, fechaFin: string): Promise<any[]> {
   ...
   */
  private async getTicketsByPriority(fechaInicio: string, fechaFin: string): Promise<any[]> {
    const [rows] = await this.query<any[]>(`
      SELECT 
   ...
   */
  private async getTicketsByCategory(fechaInicio: string, fechaFin: string): Promise<any[]> {
    const [rows] = await this.query<any[]>(`
      SELECT 
   ...
   */
  private async getTicketsByArea(fechaInicio: string, fechaFin: string): Promise<any[]> {
    const [rows] = await this.query<any[]>(`
      SELECT 
        a.nombre as area,
        a.codigo,
   ...
   */
  private async getTopTechnicians(fechaInicio: string, fechaFin: string): Promise<any[]> {
    const [rows] = await this.query<any[]>(`
      SELECT 
        u.id,
   ...
   */
  private async getSatisfactionMetrics(fechaInicio: string, fechaFin: string): Promise<any> {
    const result = await this.queryOne<any>(`
      SELECT 
   ...
   * Obtener tendencias
   */
  private async getTrends(fechaInicio: string, fechaFin: string): Promise<any> {
    // Tickets por día
    const [ticketsPorDia] = await this.query<any[]>(`
      SELECT 
   ...
   */
  private async generateAlerts(metricsGeneral: any, slaCompliance: any): Promise<any[]> {
    const alerts: any[] = [];
   ...
   */
  private async saveReport(report: any, tipoReporte: string): Promise<void> {
    await this.query(`
   ...
   */
  async exportToJson(reportId: number): Promise<any> {
    const report = await this.queryOne<any>(`
