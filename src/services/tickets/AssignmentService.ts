...
export class AssignmentService extends BaseRepository<any> {
  constructor() {
    super();
  }
  ...
  async assignTechnician(
    ...
    try {
      ...
      if (!tecnicos || (tecnicos as any[]).length === 0) {
        logger.warn(`⚠️ No hay técnicos disponibles para el ticket ${ticketId}`);
        return null;
      }

      const tecnico = tecnicos[0];

      logger.info(`✅ Técnico seleccionado: ${tecnico.nombre} ${tecnico.apellido} (Carga: ${tecnico.carga_actual}/${tecnico.max_tickets})`);

      return tecnico.id;
    } catch (error: any) {
      logger.error('❌ Error al asignar técnico:', error);
      return null;
    }
  }
  ...
  async reassignTicket(
    ...
    logger.info(`✅ Ticket ${ticketId} reasignado al técnico ${nuevoTecnicoId}`);
  }

  /**
   * Obtener carga de trabajo de un técnico
   */
  async getTechnicianWorkload(tecnicoId: number): Promise<any> {
