 ...
 */
class MetricCalculationJob extends BaseRepository<any> {
  ...
  async calculateMetrics(): Promise<void> {
    if (this.isRunning) {
      logger.warn('⚠️ Metric Calculation Job ya está en ejecución, saltando...');
      return;
    }
    ...
    try {
      ...
      logger.info(`✅ Cálculo de métricas completado para ${fechaStr}`);
    } catch (error: any) {
      logger.error('❌ Error en Metric Calculation Job:', error);
    } finally {
   ...
   */
  private async calculateTechnicianMetrics(fecha: string): Promise<void> {
    try {
      logger.info('👨‍💻 Calculando métricas de técnicos...');
      ...
      for (const tecnico of tecnicos) {
        ...
        const existing = await this.queryOne<any>(
          'SELECT id FROM metrica_tecnico WHERE tecnico_id = ? AND DATE(fecha) = ?',
          [tecnico.id, fecha]
        );

        if (existing) {
          logger.debug(`Métrica ya existe para técnico ${tecnico.id}, actualizando...`);
      ...
      logger.info(`✅ Métricas de ${tecnicos.length} técnicos calculadas`);
    } catch (error: any) {
      logger.error('Error calculando métricas de técnicos:', error);
      throw error;
    }
  }
   ...
   */
  private async createTechnicianMetric(tecnicoId: number, fecha: string): Promise<void> {
    const metricas = await this.queryOne<any>(`
      SELECT 
   ...
   */
  private async updateTechnicianMetric(tecnicoId: number, fecha: string): Promise<void> {
    const metricas = await this.queryOne<any>(`
      SELECT 
        COUNT(*) as tickets_asignados,
        COUNT(CASE WHEN t.estado_id = 5 THEN 1 END) as tickets_resueltos,
   ...
   */
  private async calculateAreaMetrics(fecha: string): Promise<void> {
    try {
      logger.info('🏢 Calculando métricas de áreas...');
      ...
      for (const area of areas) {
        ...
        const existing = await this.queryOne<any>(
          'SELECT id FROM metrica_area WHERE area_id = ? AND DATE(fecha) = ?',
          [area.id, fecha]
        );
      ...
      logger.info(`✅ Métricas de ${areas.length} áreas calculadas`);
    } catch (error: any) {
      logger.error('Error calculando métricas de áreas:', error);
      throw error;
    }
  }
   ...
   */
  private async calculateGlobalMetrics(fecha: string): Promise<void> {
    try {
      ...
      // Verificar si ya existe
      const existing = await this.queryOne<any>(
        'SELECT id FROM metrica_global WHERE DATE(fecha) = ?',
        [fecha]
      );

      if (existing) {
        await this.execute(
          `UPDATE metrica_global 
           SET total_tickets = ?,
               tickets_nuevos = ?,
               tickets_resueltos = ?,
               tickets_cerrados = ?,
      ...
      logger.error('Error calculando métricas globales:', error);
      throw error;
    }
  }
   ...
   */
  private async calculateCategoryMetrics(fecha: string): Promise<void> {
    try {
      logger.info('📂 Calculando métricas de categorías...');

      // No se guardan en tabla, solo para vistas materializadas
      logger.info('✅ Métricas de categorías actualizadas (vistas materializadas)');
    } catch (error: any) {
      logger.error('Error calculando métricas de categorías:', error);
    }
  }
  ...
  private async cleanOldMetrics(): Promise<void> {
    try {
      ...
      logger.info(`🧹 Métricas anteriores a ${fechaStr} eliminadas`);
    } catch (error: any) {
      logger.error('Error limpiando métricas antiguas:', error);
    }
  }
  ...
  start(): void {
    ...
    const schedule = config.jobs?.metricCalculationSchedule || '0 0 * * *';
    
    cron.schedule(schedule, async () => {
      await this.calculateMetrics();
    });

    logger.info(`✅ Metric Calculation Job iniciado - Schedule: ${schedule}`);
  }
}

export const metricCalculationJob = new MetricCalculationJob();